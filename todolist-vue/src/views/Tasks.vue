<template>
  <div>
    ...
    <div class="tasks-container">
      <el-card class="box-card" v-for="(item,id) in tasks" :key="item.task_id" ref="boxCards">
        <div class="task-item">
          <el-button size="mini" class="finishButton" circle @mouseover.native="hoverButton(id)"
                     @mouseleave.native="leaveButton(id)" @click="changeTaskStatus(item.task_id,id, 1)">
          ...
          <el-dropdown class="moreButton">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item divided>
                <el-link type="primary" @click="deleteTask(item.task_id, id, 1)">删除任务</el-link>
              </el-dropdown-item>
      ...
      <div>
        <el-input v-model="new_task" placeholder="请输入内容"></el-input>
        <el-button @click="addTask">添加</el-button>
      </div>


      <el-button class="finishedButton" size="mini" @click="loadFinishedTasks"><i class="el-icon-arrow-right"
                                                                                  ref="finishedButtonIcon"></i> 已完成
      ...
      <el-card class="box-card" v-if="showFinished" v-for="(item, id) in finishedTasks" :key="item.task_id">
        <div class="task-item">
          <el-button size="mini" class="finishButton" circle @click="changeTaskStatus(item.task_id, id, 0)">
          ...
          <el-dropdown class="moreButton">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item>创建时间:{{ item.task_createtime }}</el-dropdown-item>
              <el-dropdown-item divided>
                <el-link type="primary" @click="deleteTask(item.task_id, id, 0)">删除任务</el-link>
              </el-dropdown-item>
              <el-dropdown-item divided>螺蛳粉</el-dropdown-item>
              <el-dropdown-item divided>蚵仔煎</el-dropdown-item>
            </el-dropdown-menu>
...
export default {
  ...
  methods: {
    ...
    changeTaskStatus(task_id, index, status) { //status=0，代表对已完成任务操作，1为对未完成任务操作
      let param = new FormData;
      ...
      this.$axios.post("/geeker/api/update", param).then(res => {
        if (status) {
          ...
          let item = this.finishedTasks[index];
    ...
    addTask() {
      ...
      this.$axios.post("/geeker/api/insertTask", param).then(res => {
        let index = res.data.result.length-1;
        this.tasks.push(res.data.result[index]);
        this.new_task = "";
      })
    },
    load() {
    ...
    deleteTask(task_id, index, status) {
      let param = new FormData;
      param.append("token", this.$store.state.user.token);
      param.append("task_id", task_id);
      this.$axios.post("/geeker/api/deleteTask", param).then(res => {
        if (status) {
          this.tasks.splice(index, 1);
        } else {
          this.finishedTasks.splice(index, 1);
